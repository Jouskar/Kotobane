...
no network request during transcription. The production launcher starts a
small bundled launch shim inside an explicit macOS deny-network sandbox. The
shim confirms sandbox admission separately, keeps an exec-status descriptor
open with `FD_CLOEXEC`, and reports a structured errno if `execve` cannot
install the helper image. The launcher starts request writing only after both
sandbox admission and successful exec are confirmed. Sandbox installation and
helper exec failures are distinct, non-retryable errors. The helper also
receives an environment that enables offline model loading. After a confirmed
helper exec, a crash or timeout preserves the recording, restarts the helper
once, and then returns actionable diagnostics.
